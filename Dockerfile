...
ENV ORACLE_COOKIE       "Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com%2F; oraclelicense=accept-securebackup-cookie"
...
RUN apt-get update -qq &&\
    apt-get install -q -y --no-install-recommends curl ca-certificates build-essential &&\
    curl -# -H '$ORACLE_COOKIE' -kL $JAVA_URL | tar -xz -C /usr/local &&\
